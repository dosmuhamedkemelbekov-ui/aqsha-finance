...
type Modal = { type: 'transaction'; kind: Kind; transaction?: Transaction } | { type: 'recurring' } | { type: 'goal' } | null
...
export default function App() {
  const [transactions, setTransactions] = useState<Transaction[]>(() => read('aqsha-transactions-v3', []))
  const [recurring, setRecurring] = useState<Recurring[]>(() => read('aqsha-recurring-v3', []))
  const [goals, setGoals] = useState<Goal[]>(() => read('aqsha-goals-v2', []))
  const [profile, setProfile] = useState<Profile>(() => read('aqsha-profile-v2', { name: 'Пользователь', budget: 0 }))
  ...
  useEffect(() => save('aqsha-transactions-v3', transactions), [transactions])
  useEffect(() => save('aqsha-recurring-v3', recurring), [recurring])
  useEffect(() => save('aqsha-goals-v2', goals), [goals])
  useEffect(() => save('aqsha-profile-v2', profile), [profile])
  useEffect(() => {
  ...
  const exportData = () => {
    const blob = new Blob([JSON.stringify({ version: 2, transactions, recurring, goals, profile }, null, 2)], { type: 'application/json' })
  ...
  return <div className="app-shell">
    ...
    <nav className="mobile-nav">{nav.map(([label, Icon]) => <button key={label} className={active === label ? 'active' : ''} onClick={() => setActive(label)}><Icon size={20}/><span>{label}</span></button>)}</nav>
    ...
    {modal?.type === 'transaction' && <TransactionModal
      ...
      submit={data => {
        if (modal.transaction) {
          setTransactions(v => v.map(item => item.id === modal.transaction?.id ? { ...item, ...data } : item))
          notify('Операция изменена')
        } else {
          setTransactions(v => [{ ...data, id: Date.now(), createdAt: new Date().toISOString() }, ...v])
          notify(data.kind === 'income' ? 'Доход добавлен' : 'Расход добавлен')
        }
        setModal(null)
      }}
    />}
...
function Dashboard({ totals, budget, categories, recurring, transactions, openTx, setActive, togglePaid }: { totals: { income: number; expense: number; balance: number }; budget: number; categories: [string, number][]; recurring: Recurring[]; transactions: Transaction[]; openTx: (k: Kind) => void; setActive: (v: string) => void; togglePaid: (id: number) => void }) {
  return <>{!transactions.length && <section className="welcome-card"><div className="welcome-icon"><WalletCards size={25}/></div><div><p className="eyebrow">НАЧНЕМ С ЧИСТОГО ЛИСТА</p><h2>Сколько вы зарабатываете?</h2><p>Внесите зарплату или другой доход. После этого AQSHA покажет ваш настоящий баланс.</p></div><button onClick={() => openTx('income')}><Plus size={18}/> Внести зарплату</button></section>}<section className="balance-card"><div className="balance-top"><div><span className="muted-light">Общий баланс</span><h2>{money(totals.balance)}</h2><span className="balance-note">Доходы минус расходы</span></div><div className="balance-actions"><button onClick={() => openTx('income')}><ArrowDownLeft size={19}/> Доход</button><button onClick={() => openTx('expense')}><ArrowUpRight size={19}/> Расход</button></div></div><div className="balance-bottom"><div><span>Доходы</span><strong>{money(totals.income)}</strong></div><div><span>Расходы</span><strong>{money(totals.expense)}</strong></div><div className="mini-chart"><i/><i/><i/><i/><i/><i/><i/></div></div></section>
  ...
  <div className="card recurring-card"><div className="card-head"><div><p className="eyebrow">РЕГУЛЯРНЫЕ ПЛАТЕЖИ</p><h3>Ближайшие</h3></div><button className="text-button" onClick={() => setActive('Платежи')}>{recurring.length ? 'Все платежи' : 'Добавить'}</button></div>{recurring.slice(0,3).map(item => { const Icon = categoryIcons[item.category] || CalendarDays; return <button className={`recurring-item ${item.paid ? 'paid' : ''}`} key={item.id} onClick={() => togglePaid(item.id)}><span className="transaction-icon"><Icon size={20}/></span><span className="item-info"><b>{item.title}</b><small>{item.paid ? 'Оплачено' : `${item.day} числа`}</small></span><strong>{money(item.amount)}</strong></button> })}{!recurring.length && <p className="hint">Здесь появятся аренда, связь и другие ежемесячные расходы.</p>}</div></section>
...
function TransactionList({ items, onDelete, onEdit }: { items: Transaction[]; onDelete?: (id: number) => void; onEdit?: (item: Transaction) => void }) { return <div className="transaction-list">{items.length ? items.map(t => { const Icon = categoryIcons[t.category] || CreditCard; return <div className="transaction" key={t.id}><span className={`transaction-icon ${t.kind}`}><Icon size={20}/></span><span className="item-info"><b>{t.title}</b><small>{t.category} · {dateLabel(t.createdAt)}</small></span><strong className={t.kind}>{t.kind === 'income' ? '+' : '−'} {money(t.amount)}</strong>{onEdit && <button className="edit-icon tx-edit" aria-label={`Изменить ${t.title}`} onClick={() => onEdit(t)}><Pencil size={15}/></button>}{onDelete && <button className="danger-icon tx-delete" aria-label={`Удалить ${t.title}`} onClick={() => onDelete(t.id)}><Trash2 size={15}/></button>}</div> }) : <Empty text="Операций пока нет. Добавьте зарплату или первый расход."/>}</div> }
...
function TransactionModal({ kind, transaction, close, submit }: { kind: Kind; transaction?: Transaction; close: () => void; submit: (v: Omit<Transaction, 'id'|'createdAt'>) => void }) {
  const [type, setType] = useState(transaction?.kind || kind); const [amount, setAmount] = useState(transaction ? String(transaction.amount) : ''); const [title, setTitle] = useState(transaction?.title || ''); const [category, setCategory] = useState(transaction?.category || (type === 'income' ? 'Доход' : 'Продукты'))
  return <ModalShell title={transaction ? 'Изменить операцию' : type === 'expense' ? 'Добавить расход' : 'Добавить доход'} close={close}><form onSubmit={e => { e.preventDefault(); if (+amount > 0 && title.trim()) submit({ kind: type, amount: +amount, title: title.trim(), category }) }}><div className="segmented"><button type="button" className={type === 'expense' ? 'active' : ''} onClick={() => { setType('expense'); setCategory('Продукты') }}>Расход</button><button type="button" className={type === 'income' ? 'active' : ''} onClick={() => { setType('income'); setCategory('Доход') }}>Доход</button></div><Field label="Сумма"><div className="amount-input"><input required autoFocus type="number" min="1" inputMode="numeric" value={amount} onChange={e => setAmount(e.target.value)} placeholder="0"/><span>₸</span></div></Field><Field label="Название"><input required value={title} onChange={e => setTitle(e.target.value)} placeholder={type === 'income' ? 'Например, зарплата' : 'Например, продукты'}/></Field><Field label="Категория"><select value={category} onChange={e => setCategory(e.target.value)}>{(type === 'income' ? incomeCategories : expenseCategories).map(x => <option key={x}>{x}</option>)}</select></Field><button className="submit">{transaction ? 'Сохранить изменения' : 'Сохранить операцию'}</button></form></ModalShell>
}
